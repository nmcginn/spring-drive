...
You are doing tonight's increment of work on the Spring Drive explainer. Nathan reviews it in the morning, so the deliverable is **one pull request they can review over coffee**, and that PR is one complete milestone.
...
- **Nathan has answered an open question**, in the roadmap or in a PR comment. Record the answer in `docs/DECISIONS.md`, add a *Decided* note to the task it unblocks, and delete the question. That goes in tonight's PR.

Then take the first unchecked task whose Needs are met and that is not waiting on an open question.

If that milestone really is too large, take the smallest **complete** slice: one Nathan meets as a finished change rather than a staging post. Split the rest into new roadmap tasks. Slice at a seam: another widget, another sim module, an open design question. Never slice a single behaviour into an honest half tonight and a useful half tomorrow. A widget that animates but ignores reduced motion is half a widget.
...
Before you write the PR, read every roadmap entry you are about to add and ask one question of it: **would Nathan expect this to be in the PR already?**
