...
- Current milestone: M3 (not started). M0 merged as #2, M1 as #5, M2 as #6.
- Last session: 2026-09-26, M2 averaged mode and CLI. Then the maintainer answered both open questions (decisions 25 and 26).
- Open PRs waiting on review: none.
- Next up: M3 (`runaway` and `hero-glide`). M4 to M8 are all eligible too, since M1 and M2 have merged. M3 is the first widget; its screenshots go out as the CI artifact (decision 25).
